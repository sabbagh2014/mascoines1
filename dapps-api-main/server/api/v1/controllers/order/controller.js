    ...
    try {
      ...
      if (!orderRes) {
        ...
        if (!userEarningResult) {
          ...
          userEarn.massBalance = Number(orderRes.startingBid);
          await createEarning(userEarn);
        } else {
          await updateEarning(
            { _id: userEarningResult._id },
            { massBalance: Number(nftRes.startingBid) }
          );
        }
        if (userRes.userType == "Creator") {
          ...
          updateQuery.$inc = { massBalance: Number(nftRes.startingBid) };
          updateQuery1.$inc = { massBalance: -Number(nftRes.startingBid) };
